...
// Upload multipart/form-data (untuk file upload materi)
async function upload(path, formData) {
  ...
  const res = await fetch(`${BASE}${path}`, {
    ...
    headers: {
      ...(token ? { Authorization: `Bearer ${token}` } : {}),
      // JANGAN set Content-Type — biarkan browser set boundary multipart otomatis
    },
    body: formData,
  });
...
export const materiApi = {
  list:         ()             => get('/materi'),
  // Upload file sungguhan (FormData multipart)
  uploadFile:   (formData)     => upload('/materi', formData),
  // Simpan URL eksternal (JSON biasa)
