...
function ModalTambahUser({ onClose, onSuccess }) {
  ...
  async function handleSubmit() {
    ...
    // Auto-generate inisial & tag (field wajib di backend)
    const inisial = form.nama.trim()
      .split(' ').slice(0, 3).map(w => w[0]).join('').toUpperCase();
    const tag = `@${form.username.trim().toLowerCase()}`;
